...
namespace water3.Services
{
    ...
    public class SubscriberInvoicePrintService
    {
        ...
        private string BuildCompanyContactLine(CompanyInfo company)
        {
            List<string> parts = new List<string>();

            if (!string.IsNullOrWhiteSpace(company.Address))
                parts.Add("العنوان: " + company.Address);

            if (!string.IsNullOrWhiteSpace(company.Phone))
                parts.Add("الهاتف: " + company.Phone);

            if (!string.IsNullOrWhiteSpace(company.Mobile) && company.Mobile != company.Phone)
                parts.Add("الجوال: " + company.Mobile);
        ...
        private void DrawInvoice(Graphics g, Rectangle bounds, CompanyInfo company, InvoiceData inv)
        {
            ...
            Rectangle arRect = new Rectangle(x, y + 2, (w / 2) - 45, 58);
            Rectangle enRect = new Rectangle(x + (w / 2) + 45, y + 2, (w / 2) - 45, 58);
            Rectangle logoRect = new Rectangle(logoX, y, logoSize, logoSize);

            using (SolidBrush titleBrush = new SolidBrush(DarkBlue))
            {
                g.DrawString(company.CompanyName, arTitleFont, titleBrush, arRect, center);

                if (!string.IsNullOrWhiteSpace(company.CompanyNameEn))
                    g.DrawString(company.CompanyNameEn, enTitleFont, titleBrush, enRect, center);
                else
                    g.DrawString(company.SystemName, enTitleFont, titleBrush, enRect, center);
            }
            ...
            {
                g.DrawRectangle(Pens.Gray, logoRect);
                g.DrawString("Logo", smallFont, Brushes.Gray, logoRect, center);
            }
            ...
            if (string.IsNullOrWhiteSpace(contactLine))
                contactLine = company.SystemName;
            ...
            DrawCell(g, titleRect, company.InvoiceTitle, normalFont, Blue, Brushes.White, true, center);
            ...
            string note1 = string.IsNullOrWhiteSpace(company.InvoiceFooter)
   ? "• نرجو سرعة تسديد الفاتورة في مدة أقصاها 3 أيام من إصدارها."
   : "• " + company.InvoiceFooter;

            string note2 = "• يرجى مراجعة إدارة المشروع عند وجود أي ملاحظة على الفاتورة.";
        ...
        private CompanyInfo LoadCompanyInfo()
        {
            ...
            try
            {
                ...
                info.CompanyNameEn = AppSettingsService.Get("Company.NameEn", "");
                ...
                info.InvoiceTitle = AppSettingsService.Get("Invoice.Title", "فاتورة استهلاك مياه");
                info.InvoiceFooter = AppSettingsService.Get("Invoice.Footer", "");
            }
            catch
            {
                ...
                info.LogoPath = "";
                info.Currency = "ريال";
                info.InvoiceTitle = "فاتورة استهلاك مياه";
                info.InvoiceFooter = "";
            }
