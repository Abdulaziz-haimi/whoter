...
namespace water3.Reports
{
    ...
    public partial class PaymentsReportForm : BaseReportForm
    {
        private DateTimePicker dtFrom;
            ...
            public PaymentsReportForm()
            {
                Text = "تقرير المدفوعات";
                lblTitle.Text = Text;
                ApplyReportPermission("REPORT_PAYMENTS_VIEW");
                BuildFilters();
                LoadSubscribers();
            }
            ...
            protected override void LoadReportData()
            {
                try
                {
                    ...
                    dgvReport.DataSource = ReportsService.GetPaymentsReport(dtFrom.Value, dtTo.Value, selectedId > 0 ? (int?)selectedId : null);
                    lblStatus.Text = "تم تحميل تقرير المدفوعات.";
                    ReportsService.LogReportOpen("PaymentsReport", $"From={dtFrom.Value:yyyy-MM-dd}, To={dtTo.Value:yyyy-MM-dd}");
                }
                catch (Exception ex)
                {
