...
{
    //public partial class MobileSyncBatchesReportForm : Form
    //{
   
        public partial class MobileSyncBatchesReportForm : BaseReportForm
        {
            ...
            public MobileSyncBatchesReportForm()
            {
                Text = "تقرير دفعات المزامنة الجوالة";
                lblTitle.Text = Text;
                ApplyReportPermission("REPORT_MOBILE_BATCHES_VIEW");
                BuildFilters();
                LoadCollectors();
            }
            ...
            protected override void LoadReportData()
            {
                try
                {
                    ...
                    dgvReport.DataSource = _service.GetMobileSyncBatches(dtFrom.Value, dtTo.Value, selectedId > 0 ? (int?)selectedId : null);
                    lblStatus.Text = "تم تحميل تقرير دفعات المزامنة.";
                    _service.LogReportOpen("MobileSyncBatchesReport");
                }
                catch (Exception ex)
                {
