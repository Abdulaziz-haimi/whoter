...
{
    //public partial class CollectorSubscribersReportForm : Form
    //{


        public partial class CollectorSubscribersReportForm : BaseReportForm
        {
            ...
            public CollectorSubscribersReportForm()
            {
                Text = "تقرير تخصيص المشتركين للمحصلين";
                lblTitle.Text = Text;
                ApplyReportPermission("REPORT_COLLECTOR_SUBSCRIBERS_VIEW");
                BuildFilters();
                LoadCollectors();
            }
            ...
            protected override void LoadReportData()
            {
                try
                {
                    ...
                    dgvReport.DataSource = _service.GetCollectorSubscribers(selectedId > 0 ? (int?)selectedId : null);
                    lblStatus.Text = "تم تحميل تقرير تخصيص المشتركين.";
                    _service.LogReportOpen("CollectorSubscribersReport");
                }
                catch (Exception ex)
                {
