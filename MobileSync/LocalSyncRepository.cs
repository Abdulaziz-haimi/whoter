...
(
    ...
    $LastSyncedAt
                ...
                cmd.Parameters.AddWithValue("$LastInvoiceDate", x.LastInvoiceDate?.ToString("yyyy-MM-dd") ?? (object)DBNull.Value);
...
(
    ...
    $IsActive, $IsPrimary, $LinkedAt
                ...
                cmd.Parameters.AddWithValue("$LinkedAt", x.LinkedAt?.ToString("O") ?? (object)DBNull.Value);
                await cmd.ExecuteNonQueryAsync(cancellationToken);
            }
        }
